# Inventory

This proogram reads from the text file inventory.txt and makes a program that tracks inventory for the store mananger. It performs stock-taking to keep track of shoes in a store.

You can search the product by code
Determine the product with the lowest quantity and restock it. 
Determine the product with the highest quantity. 
Calculate the total value of each stock item. The total value is calculated by 
multiplying the cost by the quantity for each item entered into the system. 
